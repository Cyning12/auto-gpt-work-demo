...
- ``build_chunks_from_parsed_markdown_file`` / ``export_parsed_markdown_chunks_for_doc_paths``：对已导出 MD 批量切片并写 JSON（外部传入 ``.doc/.docx`` 相对路径列表）。
- ``load_word_chunk_records_from_chunks_json_dir`` / ``chunks_json_dir_to_faiss_chunks`` / ``persist_word_chunks_to_faiss``：合并 ``*_chunks.json`` 后调用 ``langchain_faiss_store.process_text_with_splitter`` 落盘 ``index.faiss``（与 ``langchain_rag.process_text_with_splitter`` 同源）。**PDF 源目录**增量同步请用 ``langchain_faiss_store.sync_vector_store``，二者场景不同。
"""
...
def export_parsed_markdown_chunks_for_doc_paths(
    ...
    *,
    ...
    """
    对 ``doc_rel_paths`` 中每个相对路径（通常为 ``.doc`` / ``.docx``，与知识库根目录 ``base_dir`` 拼接），
    读取 ``parsed_md_dir / {stem}.md``，切片后写入 ``chunks_json_dir / {stem}_chunks.json``。

    调用方自行传入路径列表（例如分类结果中的 ``classified["doc"]``），本函数不依赖分类字典结构。
    ...
    ok_paths: list[str] = []
    errors: list[str] = []
    for doc_idx, rel in enumerate(doc_rel_paths, start=1):
        stem = Path(rel).stem
...
def load_word_chunk_records_from_chunks_json_dir(
    chunks_json_dir: str | Path,
) -> list[dict[str, Any]]:
    """
    读取目录下所有 ``*_chunks.json``，合并为
    ``[{"content": str, "metadata": dict}, ...]``（按文件名排序）。
    """
    d = Path(chunks_json_dir)
    if not d.is_dir():
        return []
    records: list[dict[str, Any]] = []
    for fp in sorted(d.glob("*_chunks.json")):
        try:
...
def chunks_json_dir_to_faiss_chunks(chunks_json_dir: str | Path) -> list[dict[str, Any]]:
    """
    自 ``*_chunks.json`` 目录得到可直接交给 ``persist_word_chunks_to_faiss`` 的列表
    （去掉空 ``content``，避免嵌入失败）。
    """
    out: list[dict[str, Any]] = []
    for row in load_word_chunk_records_from_chunks_json_dir(chunks_json_dir):
        text = (row.get("content") or "").strip()
        if not text:
            continue
        out.append({"content": text, "metadata": dict(row["metadata"])})
    return out


def persist_word_chunks_to_faiss(
    ...
    *,
    ...
    save_merged_chunk_json: bool = True,
) -> Any:
    """
    将 Word 切片（``content`` + ``metadata``）全量向量化并写入本地 FAISS，流程对齐
    ``langchain_rag.process_text_with_splitter`` → ``langchain_faiss_store.process_text_with_splitter``。

    落盘：``{save_dir}/{index_name}.faiss``、``.pkl``；可选将合并分块写入
    ``{save_dir}/{目录名}_chunk.json``（与 PDF 管线 ``save_pdf_chunks_json`` 一致）。
    """
    if not chunks:
        raise ValueError("chunks 为空，无法建库")
    # 延迟导入，避免仅做 Word 解析时拉起 FAISS
    from langchain_faiss_store import (
        FAISS_INDEX_NAME,
        process_text_with_splitter,
        save_pdf_chunks_json,
    )

    out = Path(save_dir)
    out.mkdir(parents=True, exist_ok=True)
    idx = index_name if index_name is not None else FAISS_INDEX_NAME
    if save_merged_chunk_json:
        save_pdf_chunks_json(chunks, save_dir=out)
