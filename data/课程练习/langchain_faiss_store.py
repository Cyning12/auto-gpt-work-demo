...
PDF 按页抽取与 ``build_chunks_for_pdf`` / ``read_pdf_files`` / ``get_file_hash`` 实现在 ``pdf_file_utils``，
本模块自该处导入以保持 API 兼容。
...
from pdf_file_utils import build_chunks_for_pdf, get_file_hash, read_pdf_files
...
def chunks_dicts_from_faiss(vs: FAISS) -> list[dict[str, Any]]:
    """从 FAISS 文档库导出与 ``pdf_file_utils.read_pdf_files`` 一致结构的列表，用于写 chunk JSON。"""
    seen: set[str] = set()
